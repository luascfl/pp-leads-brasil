# Leads Brasil Printed CLI Agent Guide

This directory is a generated `leads-brasil-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
leads-brasil-pp-cli which "<capability>" --json
leads-brasil-pp-cli <command> --help
```
...
```bash
leads-brasil-pp-cli <command> --agent
```
...
```bash
leads-brasil-pp-cli <command> --help
leads-brasil-pp-cli <command> --dry-run --agent
```
