...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "leads-brasil-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LEADS_BRASIL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LEADS_BRASIL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "leads-brasil-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/leads-brasil-pp-cli/feedback.jsonl.
When ` + "`LEADS_BRASIL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LEADS_BRASIL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  leads-brasil-pp-cli feedback list
  leads-brasil-pp-cli feedback list --limit 5
  leads-brasil-pp-cli feedback list --json`,
