...
```bash
# Dry-run, no Telegram read, writes optional local evidence JSON.
...
python3 organizejr-pp-leads/ploomes_crm/crm_sheet.py telegram-people search --term ENGETOP --lead-slug engetop --execute-read
```

The bridge uses `/home/lucas/Downloads/manage_telegram` by default or `MANAGE_TELEGRAM_PATH` when set. Do not copy Telegram automation into this repo; consume the bridge JSON as local enrichment evidence. Contact saving stays explicit via `telegram-people apply-save ... --yes`.
