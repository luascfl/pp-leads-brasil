...
- Atualização ENGETOP/Gabriella: `whatsapp_export.py export --lead ENGETOP --contact Gabriella --mode full` retornou `ok=true`, `used_preferred_filter=true`, `messages=51`, `conversation_key=whatsapp:engetop:gabriella-engetop`. Transcript privado: `../organizejr-pp-leads/leads/engetop/whatsapp/gabriella/full-2026-07-30.md`. O upload equivocado de `full-2026-07-30.md` na aba `Arquivos` do Perplexity Space foi removido; após a remoção, a UI voltou a `Files`, `12 itens`, sem listar o arquivo. O envio correto pendente é colar o transcript integral no tópico ENGETOP.
